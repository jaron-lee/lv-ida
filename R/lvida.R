...
find.sink2 <- function(gm) {
    ...
    uncon <-
        which(colSums(gm) == 0) # added 2.27.2017 to speed things up
...
is.path <- function(a, b, g, internal = FALSE) {
    ...
    if (nrow(ind1) == 0)
        return(FALSE) # addition: if there are no tails at A aside from the A-->B edge
...
cond2 <- function(tmp_old, pag, q) {
    ...
    xy <-
        ind.a[which(ind == q), ] # a two-element list which is the i,j coordinate of an element in ind
    # xy[1] is the i coordinate
    # xy[2] is the j coordinate
    # can do things like pag[xy[1],xy[2]]
    if (any(tmp_old[, xy[2]] == 2)) {
        ...
        for (f in cd.ind) {
            cd.xy <-
                cd.ind.a[which(cd.ind == f), ] # cd.xy[1] is i coordinate, cd.xy[2] is j
            ...
            if (tmp_old[cd.xy[2], cd.xy[1]] == 2) {
                ...
                return(FALSE) # if neither of these if-statements are entered... (something is wrong!)
        }
        ...
        if (all(d != path)) {
            if (pag[c, d] == 0 && pag[d, c] == 0) {
                ...
                if (pag[first.pos, d] == 2 && pag[d, c] == 2 && pag[c, d] == 3) {
                    founddpath <-
                        append(founddpath, is.discr.path(path = c(path, d), pag = pag))
                    if (any(founddpath) == TRUE)
                        break
                } ## else : keep 'pag'
            }
        } ## for( d )
...
cond3 <- function(tmp_old, pag, q) {
    ...
    c <- xy4[1]
    pag <-
        tmp_old ### !!! since is.discr.path is written with 'pag', replace that with the MAG under condisideration
    indA <-
        which((pag[b,] == 2 &
                   pag[, b] != 0) & (pag[c,] == 3 & pag[, c] == 2))
    if (length(indA) == 0)
        return(TRUE) #if indA is empty return false
    for (a in indA) {
        founddpath <- c()
        pathexists <- c()
        pathexists <-
            append(pathexists, is.discr.path(path = c(c, b, a), pag = pag))
        if (any(pathexists) == TRUE)
            return(FALSE)
    }
    return(TRUE)
}
...
listMags <-
    function(pag,
        ...
        lind <- length(ind)
        if (lind == 0)
            return(list(pag)) ### if there are no circles...
        ...
        for (i in 1:nrow(tag)) {
            for (j in 1:ncol(tag)) {
                ...
                tag[i, j] <-
                    ifelse(tag[i, j] == 1 &&
                               tag[j, i] != 1, 3, tag[i, j]) # circles on o-> and o-- become tails
                if (tag[i, j] == 1 && tag[j, i] == 1)
                    ccomp[i, j] <- ccomp[j, i] <- 1
            }
        }
        ...
        #print(ccomp_m)
        listDags <-
            allDags.fast(ccomp_m, ccomp_m, NULL) # get all possible DAGs from that pattern
        ...
        if (FALSE) {
            ...
            if (method == "local") {
                ...
                connected <- function(ccomp) {
                    if (!require("igraph"))
                        stop("Package 'igraph' must be installed!")
                    ccomp_g2 <-
                        graph.adjacency(ccomp, mode = "undirected") # turn ccomp into an igraph object
        ...
        for (k in 1:nrow(listDags)) {
            ...
            for (i in 1:p) {
                for (j in 1:p) {
                    # loop thru the list of DAGs
                    if (matrix(listDags[k, ], p, p)[i, j] == 1)
                        # where there are edges in the DAG...
                    {
                        mags[[k]][i, j] <- 2
                        mags[[k]][j, i] <-
                            3 # put those edges in a copy of the original TAG
                    }
                }
            }
        }
        ...
        bigmaglist <-
            vector(mode = "list", nMags) # default value: the list can hold max 500 mags!!!
        for (k in 1:length(mags)) {
            bigmaglist[[k]] <- mags[[k]]
        }
        i <- k + 1 # counter to fill the list called bigmaglist
        mag_t <- tmp <- tmp_old <- matrix(0, nrow(pag), ncol(pag))
        for (k in 1:length(mags)) {
            # loop over elements in the list of mags
            mag_t <- tmp <- mags[[k]]
            for (n in 1:lind) {
                ...
                for (y in 1:length(comb)) {
                    ...
                    if (any(sapply(bigmaglist, identical, tmp)))
                        next
                    if (any(sapply(bigmaglist, identical, tmp_old))) {
                        ...
                        if (cond1(tmp_old, pag, q) &&
                            cond2(tmp_old, pag, q) &&
                            cond3(tmp_old, pag, q)) {
                            # conditions for transformational equivalence....
...
is.visible <- function(a, b, g) {
    ...
    if (any(g[, a] == 2)) {
        # there is at least one C*->A
        cd.ind <-
            which(g[, a] == 2) + (nrow(g) * (a - 1)) # list of positions of variables with arrowheads into A
        ...
        for (f in cd.ind) {
            ...
            if (!(g[cd.xy[1], b] != 0 && g[b, cd.xy[1]] != 0)) {
                ...
                if (is.discr.path(c(b, a, cd.xy[1]), g))
                    return(TRUE)
                else
                    next
            }
...
dsepset.reach <- function(a, b, c, adjacency)
{
    ...
    legal.dsep <- function(r, s) {
        ...
        if ((adjacency[r[[1]], r[[2]]] == 2 &&
             adjacency[s,     r[[2]]] == 2 &&
             r[[1]] != s) &&
            (is.ancestor(s, a, adjacency) ||
             is.ancestor(s, b, adjacency)) &&
            (is.ancestor(r[[2]], a, adjacency) ||
             is.ancestor(r[[2]], b, adjacency))) {
    ...
    initialize.pdsep <- function(x, y)
        mapply(makeedge, x = x, y = y)
    ...
    for (k in dsep) {
        if (adjacency[a, k] != 0) {
            if (is.ancestor(k, a, adjacency) ||
                is.ancestor(k, b, adjacency))
                dsep <- dsep
            else
                dsep <- setdiff(dsep, k)
        } else
...
pdsepset.reach <- function(a, b, c, adjacency)
{
    ...
    legal.dsep <- function(r, s) {
        ...
        if (((adjacency[r[[1]], r[[2]]] == 2 &&
             ...
             ((adjacency[r[[1]], s] != 0 &&
               r[[1]] != s))) &&
            (is.poss.ancestor(s, a, adjacency) ||
             is.poss.ancestor(s, b, adjacency))    &&
            (is.poss.ancestor(r[[2]], a, adjacency) ||
             is.poss.ancestor(r[[2]], b, adjacency))) {
            edgeslist[[r[[2]]]] <<- setdiff(edgeslist[[r[[2]]]], s)
            makeedge(r[[2]], s)
        }
    }
...
reach <- function(a, b, c, adjacency)
{
    ...
    repeat {
        ...
        for (i in seq_along(labeled[[depth - 1]])) {
            ...
            for (j in seq_along(edgestemp))
                labeled[[depth]] <-
                union(legal.pdsep(lab.i, edgestemp[[j]]),
                      labeled[[depth]])
        }
        if (length(labeled[[depth]]) == 0)
            break
...
#'
#' @param x.pos column of x in d.mat
#' @param y.pos column of y in d.mat
#' @param mcov Covariance matrix that was used to estimate graphEst
#' @param pag  PAG matrix found by FCI **this is necessary for the time being because there isn't a way to get from the graphical object to the matrix**
#' @param method "local" - local; "global" - all MAGs. Deprecated.
#' @param nMags
#' @param localcap
#' @param possdsep
#' @param verbose Verbose logging output
#' @param mags.local
#' @param Z_i
#' @param opag
#' @param bugwatch
                   ...
                   data=NULL)
{
    ...
    if (verbose)
        cat("Loaded the adjacency matrix", "\n")
    if (method == "local") {
        ##############################
        ## local method
        ...
        if (all(pag[, x.pos] == 0) || all(pag[, y.pos] == 0))
            return(0)
        
        # set of adjacencies of x
        adj <- which(pag[, x.pos] != 0)
        if (verbose)
            cat("Calculated adjacencies of x", "\n")
        ...
        if (verbose)
            cat("Constructed set of possible descendents", "\n")
        if (!(y.pos %in% pdes))
            return(0)
        # possible d-sep
        if (possdsep == "small") {
            pdsep <-
                union(pdsepset.reach(x.pos, y.pos,-1, pag), y.pos) ##added y.pos to this set 8/29
        }
        if (possdsep == "big") {
            pdsep <-
                union(reach(x.pos, y.pos,-1, pag), y.pos) ##added y.pos to this set 8/29
        }
        if (verbose)
            cat("Constructed possible-d-sep set", "\n")
        # Z_i = union of adj, pdes, and pdsep
        Z_i <- sort(unique(c(adj, pdes, pdsep)))
        if (!is.null(localcap) && length(Z_i) > localcap) {
            cat(
                "WARNING: cannot localize when calculating the effect of ",
                x.pos,
                " on ",
                y.pos,
                ". Z_i is too big. Size of Z_i = ",
                length(Z_i),
                ". Returning NA",
                "\n"
            )
        ...
        for (i in 1:n.mags) {
            ...
            gMag_x <-
                remove.visible.edges(x.pos, gMag) ### This is the MAG when remove invisible edges out of x
...
#' Run Latent Variable-IDA - Linear Models
#'
#' Estimate the causal effect of x on y; no precomputed covariance matrix required (so can handle factors); all MAGs can be precomputed
#'
#' @param x.pos name of column of x in d.mat
#' @param y.pos name of column of y in d.mat
#' @param data data matrix or data frame (with named columns)
#' @param pag  PAG matrix found by FCI (with named rows and columns)**this is necessary for the time being because there isn't a way to get from the graphical object to the matrix**
#' @param method "local" - local; "global" - all MAGs. Deprecated.
#' @param nMags
#' @param localcap
#' @param possdsep
